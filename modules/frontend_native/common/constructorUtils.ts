...
export function updateTemperature(blocks: StepDTO[]): [StepDTO[], number] {
    ...
    for (let i = 0; i < tempBlocks.length; i++) {
        if (tempBlocks[i].type == StepType.TEMP_CHANGE) {
            ...
            if (i != 0 && tempBlocks[i - 1].type == StepType.TEMP_CHANGE) {
                ...
                editedBlock.params = {
                    source: (tempBlocks[i - 1].params as TemperatureStep)
                        .source,
    ...
    const temperatures = refactoredBlocks.filter(
        (block) => block.type == StepType.TEMP_CHANGE,
    );
    const newCurrentTemp =
        temperatures.length == 0
            ? DEFAULT_TEMEPRATURE
            : (temperatures[temperatures.length - 1].params as TemperatureStep)
                  .target;
