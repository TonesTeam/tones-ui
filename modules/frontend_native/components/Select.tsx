...
export function CustomSelect(props: CustomSelectProps) {
    ...
    const [selected, setSelected] = useState<
        LiquidDTO | LiquidTypeDTO | undefined
    >();
    ...
    const initialization = () => {
        if (props.list.length == 0) {
            ...
            setSelected(
                props.list.find(
                    (item) =>
                        JSON.stringify(item) == JSON.stringify(props.selected),
                ),
            );
        }
    };
        ...
        if (item.id == -1 && canAdd) {
            let newReagent = {
                id:
                    searchList.length == 0
                        ? 0
                        : searchList[searchList.length - 1].id + 1,
                name: item.name,
                type: { id: -1, name: 'Pseudo' },
            } as LiquidDTO;

            if (props.onCreateOption)
                props.onCreateOption(newReagent as LiquidDTO);
        } else {
                ...
                {searchList && selected != undefined && (
                    <>
                        <SelectDropdown
                            ...
                            disabledIndexs={
                                suggestAdd(filterInput)[0].id ==
                                OptionID.EMPTY_SEARCT_RESULT
                                    ? [0]
                                    : []
                            }
