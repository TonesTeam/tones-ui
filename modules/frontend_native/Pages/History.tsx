...
export default function History(props: any) {
  ...
  const filterAndSort = () => {
    if (protocols) {
      let filteredList = protocols.filter((record) => {
        const matchesDate = selectedDate ? new Date(record.creationDate).toISOString().split('T')[0] === selectedDate : true;
        const matchesSearch = filterInput === "" ? record : record.name.toLowerCase().includes(filterInput.toLowerCase());
  ...
  return (
    <MainContainer>
      <NavBar />
      <View style={[globalElementStyle.page_container, { padding: 10, flexDirection: 'row' }]}>        
        <View style={{ flex: 1 }}>
          <View style={s.section_calendar}>
            <Calendar
              ...
              markedDates={
                protocols && protocols.reduce((acc, record) => {
                  const date = new Date(record.creationDate).toISOString().split('T')[0];
                  if (!acc[date]) {
                    acc[date] = { marked: true, dotColor: AppStyles.color.primary };
                  }
                  if (selectedDate === date) {
                    acc[date].selected = true;
                    acc[date].selectedColor = AppStyles.color.primary;
                  }
                  return acc;
                }, selectedDate ? { [selectedDate]: { selected: true, selectedColor: AppStyles.color.primary } } : {})
              }
              style={{ borderRadius: 15 }}
            />
          </View>
          <View style={s.sort_container}>            
            <Txt style={{ fontFamily: "Roboto-bold", fontSize: 16, marginBottom: 5 }}>Sort By:</Txt>
            <Picker
        ...
        <View style={{ flex: 3 }}>
          <View style={s.section_search}>
            <Txt style={{ fontFamily: "Roboto-bold", fontSize: 24 }}>History</Txt>
            <View
              style={[
                s.search_bar,
          ...
          <View style={s.section_list}>
            {(protocols == undefined || liquids == undefined) && (
              <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
                ...
                <Txt style={{ fontFamily: "Roboto-thin", fontSize: 24 }}>Loading...</Txt>
              </View>
            )}
              ...
              <>
                <View style={{ flex: 1 }}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      {!networkError && (
                        <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
                          ...
                          <Txt style={{ color: AppStyles.color.text_faded, fontSize: 30, marginTop: 30 }}>
                            No records found
                          </Txt>
                        </View>
                      )}
                  ...
                  {filterAndSort().length != 0 && (
                    ...
                    >
                      {filterAndSort().map(function (record, index) {
                        return (
                          <View key={index} style={s.history_item}>
                            <Txt style={s.history_item_text}>ID: {record.id}</Txt>
                            <Txt style={s.history_item_text}>{record.name}</Txt>
                            <Txt style={s.history_item_author}>Author: {record.author}</Txt>
                          </View>
                        );
                      })}
                    </ScrollView>
