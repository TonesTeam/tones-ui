...
class Ongoing implements StatusElements {
    getMessage(): JSX.Element {
        return (
            <div>
                <img src={progressGif} style={{ height: '400px' }} />
            </div>
        );
    }
    getColor(): string {
    ...
    getMessage(): JSX.Element {
        return (
            <div className="comment-body">
                ...
                <p id="comment-header">
                    Some error occured! Please follow the guide below to detect
                    and fix the error.
                </p>
                <ol id="error-guide">
                    <li>
                        Check if any slots are displaced. If any - fix the
                        placing.
                    </li>
                    <li>Make sure that sealing slots cover is closed.</li>
                    <li>
                        Here will be a more precise fixing guide based on the
                        type of error.
                    </li>
                </ol>
            </div>
    ...
    getMessage(): JSX.Element {
        return (
            <div className="comment-body">
                ...
                <p>
                    Do stuff that you need to do and then press Discard button
                </p>
            </div>
        );
    }
...
export default function LaunchPage() {
    ...
    let findIndexByProtocol = function (protocol: ProtocolState) {
        ...
        return stateProtocols.findIndex((e) => {
            return e.protocol?.id === protocol.protocol.id;
        });
    };
    ...
    const activeProto = useAppSelector((state) => state.protocols).find(
        (e) => e.protocol.id.toString() === params.id,
    )!;
    const index = findIndexByProtocol(activeProto);
    const statusStrat = launchStatus.get(activeProto?.status);
    const activeProgress = useAppSelector(
        (state) => state.protocols[index]?.progress,
    );
    const activeStatus = useAppSelector(
        (state) => state.protocols[index]?.status,
    );
            ...
            <div className="font-rb" id="main">
                <div id="header">
                    ...
                    <div className="header-tile">
                        ...
                        <p className="info">
                            {activeProto.status.toLowerCase()}
                        </p>
                    </div>
                    <div className="header-tile">
                ...
                <div className="progress-container">
                    <div id="progress">
                        <div
                            id="progress-bar"
                            style={{
                                width: `${activeProto.progress}%`,
                                backgroundColor: statusStrat?.getColor(),
                            }}
                        ></div>
                    </div>
                </div>
                ...
                <div className="footer">
                    <div id="comment">{statusStrat?.getMessage()}</div>
