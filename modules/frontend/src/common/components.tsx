...
export function CustomSelect(props: any) {
    ...
    function handleZIndex() {
        ...
        selects.forEach((select, index) => {
            ...
            inps.forEach((inp, index2) => {
                (inp.lastChild as HTMLLabelElement).style.zIndex = String(
                    100 - index,
                );
                if ((inp.firstChild as HTMLInputElement).checked) {
                    (inp.lastChild as HTMLLabelElement).style.zIndex = String(
                        100 - index * 2,
                    ); //selected option stays on top
                } else {
                    (inp.lastChild as HTMLLabelElement).style.zIndex = String(
                        99 - index * 2,
                    );
    ...
    useEffect(() => {
        let onLoadParamPass = document.querySelectorAll(
            'input[name=' + props.opt_name + ']:checked',
        );
                        ...
                        <div key={index}>
                            {((selected != null && selected == opt.value) ||
                                ...
                                <>
                                    ...
                                    />
                                    <label
                                        htmlFor={`opt${index}_${props.opt_name}`}
                                        className="option"
                                    >
                                        {opt.label}
                                    </label>
