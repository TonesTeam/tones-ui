...
export default function MainKeyboard(props: {
    inputValue?: string;
    layout: string;
    show: boolean;
    showSetter: (s: boolean) => void;
    inputSetter: (inp: string) => void;
}) {
    ...
    const onKeyPress = (k: string) => {
        ...
        if (k == '{shift}' || k == '{lock}')
            setKeyboardLayout(layoutMap.get(keyboardLayout)!);
    };
    return (
        <>
            <div
                className={showKeyboard ? 'visible' : 'hidden'}
                style={{
                    position: 'fixed',
                    zIndex: 20,
                    width: '100%',
            ...
            >
                <style>{`.hg-theme-default .hg-button {height: 10vh}`}</style>
                <Keyboard
