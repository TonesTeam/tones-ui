...
const progressIconStyle = (actProtocols: ProtocolState[]) => {
    if (document.getElementById('progress-icon')) {
        ...
        if (
            actProtocols.find((p) => {
                return p.status == Status.Error;
            })
        ) {
            blinkIcon.className = 'fa fa-flask ERROR';
        } else if (
            actProtocols.find((p) => {
                return p.status == Status.Finished;
            })
        ) {
...
export default function NavigationBarOld(props: { selectedItem?: string }) {
    ...
    return (
        <div
            id="navbar"
            className={`sidenav ${isOpen ? 'sidenav-open' : 'sidenav-closed'} font-rb`}
        >
            {!isOpen && (
                <>
                    <CenteringFlexBox
                                ...
                                {activeProtocols.map((p) => {
                                    return (
                                        <a
                                            ...
                                            href={`/start/${p.protocol.id}`}
                                        >
                                            <div className="progress-item">
                                                <div
                                                    className={`bliking-led ${p.status}`}
                                                ></div>
