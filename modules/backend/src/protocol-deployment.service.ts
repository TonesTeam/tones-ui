...
export class ProtocolDeploymentService {
    ...
    async deployProtocol(protocolId: number): Promise<LiquidDeploymentDTO[]> {
        ...
        liquidToUseCount.forEach((v, k) => {
            ...
            getUsedHolders(amount, liquidHolders).forEach(sl => config.push({
                liquidInfoId: k,
                amount: sl.size,
                slotNumber: sl.slot,
...
export function getUsedHolders(liquidAmount: number, liquidHolders: Map<number, number>): {slot: number, size: number}[] {
    ...
    while(remainder > 0) {
        ...
        const slot = liquidConfigToMap().get(bestHolder) - liquidHolders.get(bestHolder) + prevSlots(bestHolder);
