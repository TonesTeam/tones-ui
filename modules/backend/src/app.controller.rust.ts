...
class RustProtocolManager {
    ...
    convert(prot: any): void {
        this.id = prot.id;
        this.steps = prot.steps.map((s: any) => {
            switch (s.type) {
                ...
                case "Reagent":
                    const liquid_params = {
                        reagent_type: s.params.liquid.slot !== undefined 
                            ? { External: s.params.liquid.slot } 
                            : { Internal: { x: s.params.liquid.Internal?.x ?? 0, y: s.params.liquid.Internal?.y ?? 0 } },
                        used_cold: true,
                        toxic: false
                    };
                ...
                case "Washing":
                    const wash_type = s.params.wash?.Sys === "Wash" ? "SysDefault" : { Custom: 0 };
                    ...
                    return {
                        step_type: {
                            Washing: {
                                liquid_params: {
                                    wash_type,
                                    used_cold: s.params.wash?.used_cold ?? true,
