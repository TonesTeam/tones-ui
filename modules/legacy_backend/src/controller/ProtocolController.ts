...
} from '@service/commands/Commands';
...
export default class ProtocolController extends BaseHttpController {
    ...
    public async allProtocols() {
        const protocols = await (
            await this.dbService.getRepository(Protocol)
        ).find({
            relations: ['creator', ...PROTOCOL_STEP_RELATIONS],
        });
        return this.json(
            protocols.map((p) => this.protocolMapper.toDto(p)),
            200,
        );
    }
    ...
    public async getProtocolLiquidConfiguration(
        @requestParam('id') id: string,
        @queryParam('slots') slots: string,
    ) {
        slots = slots ?? 1;
        ...
        await Promise.all(
            res.map(
                async (la) =>
                    (la.liquid = await this.liquidMapper.toDtoFromId(
                        la.liquidId,
                    )),
            ),
        );
    ...
    private async resolveProtocolConfig(
        id: string,
        slots: string,
    ): Promise<[DeploymentLiquidConfiguration[], Command[]]> {
        const protocol = await (
            await this.dbService.getRepository(Protocol)
        ).findOneOrFail(id, { relations: PROTOCOL_STEP_RELATIONS });
        const slotArray = Array.from(Array(parseInt(slots)).keys());
        const commands = protocol.steps.flatMap((s) =>
            this.protocolCommandsMapper.stepToCommand(s, slotArray),
        );
        let iOrder = 1;
        ...
        const res =
            this.liquidConfigurationResolver.resolveLiquidConfiguration(
                applicationCommands,
            );
        return [res, commands];
    }
