...
export class ProtocolXmlParsingService {
    ...
    public async createProtocolFromXml(
        protocolXml: Element,
    ): Promise<Protocol> {
        const protocol = new Protocol();
        protocol.steps = [];
        const cleanProtocolXml = this.scrubIdsFromDocument(protocolXml);
        protocol.creator = await (
            await this.dbservice.getRepository(User)
        ).findOneOrFail();
        protocol.protocolType = await (
            await this.dbservice.getRepository(ProtocolType)
        ).findOneOrFail();
        protocol.protocolName = cleanProtocolXml.querySelector(
            ':scope>field[name=protocol_name]',
        )?.innerHTML!;
        ...
        protocol.standardTemp = parseInt(
            cleanProtocolXml.querySelector(':scope>field[name=temp]')!
                .innerHTML,
        );
        const blockToParse = cleanProtocolXml.querySelector(
