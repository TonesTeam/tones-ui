...
class TubeConfigState {
    ...
    getAvailableSizes() {
        return Array.from(this.tubeSizeCountMap.entries())
            .filter((kv) => kv[1] > 0)
            .map((kv) => kv[0]);
    }
    ...
    allocateLiquidSlotOfSize(size: number) {
        ...
        let allocatedSlot =
            previousLiquidSlots + sizeCount - this.tubeSizeCountMap.get(size)!;
        if (size == 500) {
...
export class LiquidConfigurationResolver {
    public resolveLiquidConfiguration(
        commands: LiquidApplicationCommand[],
    ): DeploymentLiquidConfiguration[] {
        const state = new TubeConfigState();
        const nonWashingCommands = commands.filter(
            (la) => !la.liquidInfo.isWashing,
        );
        const nonWashingDeploymentConfig = this.getLiquidsConfiguration(
            nonWashingCommands,
            state,
        );
        const washingTubeConfig = config['liquids-tube-holders'].filter(
            (t) => t['for-washing'],
        )[0];
        state.tubeSizeCountMap.set(
            washingTubeConfig.size,
            washingTubeConfig.count,
        );
        const washingDeploymentConfig = this.getWashingConfiguration(
            commands,
            state,
        );
        return lodash.concat(
            nonWashingDeploymentConfig,
            washingDeploymentConfig,
        );
    }

    private getWashingConfiguration(
        commands: LiquidApplicationCommand[],
        state: TubeConfigState,
    ): DeploymentLiquidConfiguration[] {
        let washCommands = commands.filter((la) => la.liquidInfo.isWashing);
        return Array.from(
            groupBy(washCommands, (lac) => lac.liquidInfo.id).values(),
        ).map((locs) => this.washingCommandsToDeploymentConfig(locs, state));
    }

    private washingCommandsToDeploymentConfig(
        sameLiquidWashingCommands: LiquidApplicationCommand[],
        state: TubeConfigState,
    ): DeploymentLiquidConfiguration {
        ...
        return {
            liquid: undefined,
            liquidAmount: lodash.sum(
                sameLiquidWashingCommands.map((i) => i.volume),
            ),
            liquidId: liquid.id!,
    ...
    private getLiquidsConfiguration(
        commands: LiquidApplicationCommand[],
        state: TubeConfigState,
    ): DeploymentLiquidConfiguration[] {
    ...
    private getLiquidConfiguration(
        commands: LiquidApplicationCommand[],
        state: TubeConfigState,
    ): DeploymentLiquidConfiguration[] {
        let acc = 0;
        ...
        for (let i = 0; i < commands.length; i++) {
            ...
            if (command.volume > Math.max(...state.getAvailableSizes())) {
                throw new Error(
                    'Impossible to configure liquids for protocol deployment',
                );
            }
            acc += command.volume;
            if (state.getAvailableSizes().every((s) => acc > s)) {
                ...
                const dlc: DeploymentLiquidConfiguration = {
                    ...
                    liquidSlotNumber:
                        state.allocateLiquidSlotOfSize(maxAvailableSize),
                    liquid: undefined,
                };
                liquidConfig.push(dlc);
                hydratedCommands.forEach(
                    (c) => (c.from = dlc.liquidSlotNumber),
                );
                hydratedCommands = [];
                acc = 0;
                i--;
