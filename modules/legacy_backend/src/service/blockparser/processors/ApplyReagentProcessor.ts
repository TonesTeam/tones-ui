...
export default class ApplyReagentProcessor extends BlockProcessor {
    async process(
        applyReagent: Element,
        protocol: Protocol,
    ): Promise<Protocol> {
        const times = parseInt(
            applyReagent.querySelector(':scope> field[name=times]')!.innerHTML,
        );
        for (var i = 0; i < times; i++) {
            this.helper.appendTempStep(
                protocol,
                parseInt(
                    applyReagent.querySelector(':scope> field[name=degrees]')!
                        .innerHTML,
                ),
                true,
            );
            protocol.steps.push(
                await this.createLiquidApplicationStep(protocol, applyReagent),
            );
        }
        this.helper.appendWaitStep(
            protocol,
            parseInt(
                applyReagent.querySelector(':scope> field[name=time]')!
                    .innerHTML,
            ),
        );
        return protocol;
    ...
    private async createLiquidApplicationStep(
        protocol: Protocol,
        applyReagent: Element,
    ) {
        const s = new Step();
        ...
        s.liquidApplication.liquid = await this.helper.findLiquid({
            ...
            liquidSubType: {
                subTypeName: applyReagent.querySelector(
                    ':scope> field[name=reagent_type]',
                )!.innerHTML,
            },
            liquidName: applyReagent.querySelector(
                ':scope> field[name=reagent]',
            )!.innerHTML,
        });
        return s;
    }
