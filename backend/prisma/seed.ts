...
import { StepType, UserRole } from 'common/enums';
...
async function main() {
    ...
    const findOrCreateType = async (name: string) => {
        return await prisma.liquidType.upsert({
            where: { name },
            update: {},
            create: { name },
        });
    ...
    let amplificationType = await findOrCreateType(
    ...
    const findOrCreateLiquid = async (name: string, type: LiquidType) => {
        let lid =
            (
                await prisma.permanentLiquid.findFirst({
                    where: { liquidInfo: { name } },
                })
            )?.id ?? -1;
    ...
    let signalAmpl1 = await findOrCreateLiquid(
        'Akoya Polymer System',
        amplificationType,
    );
    let signalAmpl3 = await findOrCreateLiquid(
        'ImmPress Red Polymer System',
        amplificationType,
    );
    let signalAmpl4 = await findOrCreateLiquid(
        'ImmPress Mal Polymer System',
        amplificationType,
    );
